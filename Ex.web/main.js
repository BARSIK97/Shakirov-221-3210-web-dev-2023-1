...
document.addEventListener('DOMContentLoaded', function () {
    const apiUrl = 'http://exam-2023-1-api.std-900.ist.mospolytech.ru/api/'
        +
        'routes?api_key=287a1b11-36b0-4af6-854a-a2a9ad26525c';
    let currentPage = 1;
    ...
    function updatePaginationButtons(totalFilteredItems) {
        ...
        const createBootstrapListItem = (
            page, isDisabled = false, isCurrent = false, text = page) => {
            const listItem = document.createElement('li');
            listItem.className = `page-item ${isCurrent ? 'active' : ''}
             ${isDisabled ? 'disabled' : ''}`;
        ...
        if (startRange > 1) {
            navList.appendChild(
                createBootstrapListItem('<< Первая страница'));
        }
        if (startRange > 1) {
            navList.appendChild(createBootstrapListItem('<'));
        }
        for (let i = startRange; i <= endRange; i++) {
            navList.appendChild(
                createBootstrapListItem(i, false, currentPage === i));
        }
        ...
        if (currentPage < totalPages) {
            navList.appendChild(
                createBootstrapListItem('Последняя страница >>'));
        }
        ...
        xhr.onreadystatechange = function () {
            if (xhr.readyState === 4) {
                if (xhr.status === 200) {
                    ...
                    if (mainObjectFilter) {
                        data = data.filter(item => item.mainObject.toLowerCase()
                            .includes(mainObjectFilter.toLowerCase()));
                    }
    ...
    function navigateToPage(pageNumber) {
        if (pageNumber < 1 || pageNumber > Math.ceil(
            totalItems / itemsPerPage)) {
            console.error('Ошибка номера сстраеицы:', pageNumber);
            return;
        }
        currentPage = pageNumber;
        loadTableData(
            apiUrl, currentPage, currentSearchTerm, currentMainObjectFilter);
    }
    ...
    const objectFilterSelect = document.querySelector(
        'select[name="Основной объект"]');

    objectFilterSelect.addEventListener('change', function (e) {
        currentMainObjectFilter = e.target.value !== 'Не выбрано'
            && e.target.value !== 'Основной объект' ? e.target.value : '';
        reloadFilteredData(currentSearchTerm, currentMainObjectFilter);
    });
    ...
    loadTableData(
        apiUrl, currentPage, currentSearchTerm, currentMainObjectFilter);
    const languageFilterSelect = document.querySelector(
        'select[name="Языки экскурсии"]');
    let minWorkExperience = null;
    let maxWorkExperience = null;
    function applyFilters() {
        const languageSelected = document.querySelector(
            'select[name="Языки экскурсии"]').value;
        const guideRows = document.querySelectorAll('.realtor.table tbody tr');
        guideRows.forEach(row => {
            const guideLanguage = row.querySelector(
                'td:nth-child(3)').textContent;
            const workExperience = parseInt(row.querySelector(
                'td:nth-child(4)').textContent, 10);
            const matchesLanguage = languageSelected === 'Языки экскурсии'
                || guideLanguage.includes(languageSelected);
            const matchesMinExperience = minWorkExperience === null
                || workExperience >= minWorkExperience;
            const matchesMaxExperience = maxWorkExperience === null
                || workExperience <= maxWorkExperience;
            if (matchesLanguage && matchesMinExperience
        ...
        xhr.onreadystatechange = function () {
            if (xhr.readyState === 4 && xhr.status === 200) {
                ...
                const relatedTableBody = document.querySelector(
                    '.realtor.table.table-bordered  tbody');
                relatedTableBody.innerHTML = '';
                data.forEach(function (item) {
                    const profileImage = "images/realtors.png";
                    const row = `<tr>
                    <td><img src="${profileImage}" alt="Profile"
                    class="img-fluid profile-pic"></td>
                    <td>${item.name}</td>
                    <td>${item.language}</td>
                    <td>${item.workExperience}</td>
                    <td>${item.pricePerHour}</td>
                    <td><button type="button" class="btn btn-primary 
                    guide-select-button">Выбрать</button></td>
                    <td>${item.id}</td> 
                    </tr>`;
    ...
    tableBody.addEventListener('click', function (e) {
        if (e.target && e.target.classList.contains('select-button')) {
            ...
            const previouslySelectedRow = document.querySelector(
                '.table-active');
            ...
            e.target.classList.add('button-clicked');
            const routeName = e.target.closest('tr').querySelector(
                'td:nth-child(1)').textContent;
            const applicationFormInput = document.getElementById(
                'selectedRoute');
            ...
            const objectId = e.target.closest(
                'tr').getAttribute('data-object-id');
            selectedRouteId = objectId;
            loadRelatedData(objectId);
            document.querySelector(
                '.realtor.table.table-bordered').style.display = 'block';
    ...
    function showApplyButton() {
        ...
        if (!applyButtonContainer) {
            ...
            applyButtonContainer.className =
                'button-container mt-3 d-flex justify-content-center';
            applyButtonContainer.innerHTML =
                '<button type="button" class="btn btn-primary" data-bs-toggle='
                +
                '"modal" data-bs-target="#applyModal">Оформить заказ</button>';
            const realtorTableContainer = document.querySelector(
                '.realtor.table.table-bordered');
    ...
    const guideTableBody = document.querySelector(
        '.realtor.table.table-bordered tbody');
    guideTableBody.addEventListener('click', function (e) {
        if (e.target && e.target.classList.contains('guide-select-button')) {
            const guideButtons = document.querySelectorAll(
                '.guide-select-button');
            guideButtons.forEach(button => {
                button.classList.remove('button-clicked');
            });
            e.target.classList.add('button-clicked');
            selectedGuideId = e.target.closest(
                'tr').querySelector('td:nth-child(1)').textContent;
            const guideName = e.target.closest(
                'tr').querySelector('td:nth-child(2)').textContent;
            selectedGuideCost = parseFloat(e.target.closest(
    ...
    function calculatePrice(guideCostPerHour, hours, dayOffMultiplier,
        morningFee, eveningFee, visitors) {
        let visitorcost = 0;
        if (visitors >= 5 && visitors < 10) visitorcost = 1000;
        if (visitors >= 10 && visitors <= 20) visitorcost = 1500;
        return (guideCostPerHour * hours * dayOffMultiplier) +
            morningFee + eveningFee + visitorcost;
    }
    ...
    function checkIfDayOff(dateString) {
        ...
        const dateForCheck = ('0' + (date.getMonth() + 1)).slice(-2)
            + '-' + ('0' + date.getDate()).slice(-2);
        if (dayOfWeek === 0 || dayOfWeek === 6
            || publicHolidays.includes(dateForCheck)) {
            return 1.5;
        } else {
            return 1;
        }
    ...
    function handleFormInput() {
        ...
        const hoursNumber = parseInt(
            document.getElementById('durationSelect').value);
        const isThisDayOff = checkIfDayOff(
            document.getElementById('excursionDate').value);
        const isItMorning = checkIfMorning(
            document.getElementById('startTime').value);
        const isItEvening = checkIfEvening(
            document.getElementById('startTime').value);
        ...
        document.getElementById(
            'additionalOption1').addEventListener('change', handleFormInput);

        let price = calculatePrice(guideServiceCost, hoursNumber,
            isThisDayOff, isItMorning, isItEvening, numberOfVisitors);
        let originalPrice = price;
        if (hasDiscount) {
            price -= originalPrice * 0.25;
        }
        const hasExtraDiscount = document.getElementById(
            'additionalOption2').checked;
        if (hasExtraDiscount) {
            if (isThisDayOff === 1.5) {
                price += originalPrice * 0.25;
            } else {
                price += originalPrice * 0.30;
            }
        }
    ...
    function closeForm() {
        ...
        if (applyModal) {
            ...
            const newApplyModal = new bootstrap.Modal(
                applyModalElement, { keyboard: true });
            newApplyModal.hide();
        }
    }
    ...
    function sendApplication() {
        ...
        const price = parseFloat(
            document.getElementById('totalCost').value.replace(' руб.', ''));
        const optionFirst = document.getElementById(
            'additionalOption1').checked ? 1 : 0;
        const optionSecond = document.getElementById(
            'additionalOption2').checked ? 1 : 0;
        const date = document.getElementById('excursionDate').value;
        const time = document.getElementById('startTime').value;
        const guideRow = document.querySelector(
            '.realtor.table.table-bordered tbody .button-clicked').
